...
from CatalystGen.modules.embeddings import CONTINUOUS_EMBEDDINGS
...
# ADS_ENCODING = {"C": 0,"CC": 1,"CCH": 2,"CCH2": 3,"CCH2OH": 4,"CCH3": 5,"CCHO": 6,"CCHOH": 7,
#                 "CCO": 8,"CH2": 9,"CH2CH2OH": 10,"CH2CH3": 11,"CH2CO": 12,"CH2O": 13,"CH2OH": 14,
#                 "CH3": 15,"CH4": 16,"CHCH": 17,"CHCH2": 18,"CHCH2OH": 19,"CHCHO": 20,"CHCHOH": 21,
#                 "CHCO": 22,"CHCOH": 23,"CHOCH2OH": 24,"CHOCHO": 25,"CHOCHOH": 26,"CHOH": 27,"CHOHCH2": 28,
#                 "CHOHCH2OH": 29,"CHOHCH3": 30,"CHOHCHOH": 31,"CN": 32,"COCH2O": 33,"COCH3": 34,"COCHO": 35,
#                 "COHCH2OH": 36,"COHCH3": 37,"COHCHO": 38,"COHCHOH": 39,"COHCOH": 40,"H": 41,"N": 42,
#                 "N2": 43,"NH": 44,"NH3": 45,"NHNH": 46,"NNH": 47,"NNO": 48,"NO": 49,
#                 "NO2": 50,"NO2NO2": 51,"NO3": 52,"NONH": 53,"O": 54,"OCH2CH3": 55,"OCH2CHOH": 56,
#                 "OCH3": 57,"OCHCH3": 58,"OH": 59,"OH2": 60,"OHCH2CH3": 61,"OHCH3": 62,"OHNH2": 63,
#                 "OHNNCH3": 64,"ONH": 65,"ONNCH32": 66,"ONNH2": 67,}
...
def lattice_params_to_matrix_torch(lengths, angles):
    ...
    return torch.stack([vector_a, vector_b, vector_c], dim=1).float()
...
def frac_to_cart_coords(
    ...
    lattice = lattice_params_to_matrix_torch(lengths, angles)
    lattice_nodes = torch.repeat_interleave(lattice, num_atoms, dim=0)
    lattice_nodes = lattice_nodes.float()  # 🔧 안전하게 lattice도 변환
...
def get_pbc_distances(
    ...
    # correct for pbc
    lattice_edges = torch.repeat_interleave(lattice, num_bonds, dim=0).float()
...
def preprocess(input_file, num_workers, niggli, primitive, graph_method): 
    ...
    def process_one(row, niggli, primitive, graph_method):
        ...
        # (1) absorbate 인코딩
        if 'ads' in row and isinstance(row['ads'], str):
            clean_ads = row['ads'].replace("*", "")
            result_dict['absorbate_id'] = ADS_ENCODING.get(clean_ads, -1)

        # (2) miller index 튜플 파싱
        if 'miller' in row and isinstance(row['miller'], str):
            try:
                result_dict['miller_index'] = ast.literal_eval(row['miller'])
            except:
                result_dict['miller_index'] = None  # fallback
...
def sanitize_batch_dtype(batch):
        for key in ['frac_coords', 'lengths', 'angles', 'to_jimages', 'cell', 'cell_offsets']:
            if hasattr(batch, key):
                setattr(batch, key, getattr(batch, key).float())
            return batch
