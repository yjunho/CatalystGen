...
class GemNetT(torch.nn.Module):
    ...
    def reorder_symmetric_edges(
        ...
        #이웃이 사라지는 배치는 제거
        if torch.any(neighbors_new == 0):
            print("❌ neighbors_new has 0 entries, skipping structure")
...
# ✅ 추가된 안전 검사
        if (
            ...
            torch.isnan(D_st).any() or torch.isinf(D_st).any() or
            torch.isnan(V_st).any() or torch.isinf(V_st).any() or
            num_bonds.sum().item() == 0
        ):
            return None

        (edge_index,cell_offsets,neighbors,D_st,V_st,) = self.reorder_symmetric_edges(edge_index, to_jimages, num_bonds, D_st, V_st)
    ...
    def forward(self, z, frac_coords, atom_types, num_atoms, lengths, angles,
        ...
        graph_out = self.generate_interaction_graph(
        pos, lengths, angles, num_atoms, edge_index, to_jimages, num_bonds
    )

        # 🔸추가: generate_interaction_graph가 None을 반환한 경우 건너뛰기
        if graph_out is None:
            return None  # 상위 호출부에서 이걸 감지해 skip하도록 처리 필요
        ...
        graph_repr = scatter(h, batch, dim=0, dim_size=nMolecules, reduce="mean")
        ...
        if self.regress_forces:
            ...
            return h, F_t, graph_repr # (nMolecules, num_targets), (nAtoms, 3)
        else:
            return E_t
